...
create table 設施資訊(
    ...
    地下樓層數 number,
    人數 number,
    分局代碼 char(4) references 
        轄管分局資訊(轄管分局代碼),
    村里代號 char(4) references 
        轄管村里資訊(村里別代號)
);
...
--練習四
--4-1
select distinct B.轄管分局, B.分局電話
    from
     設施資訊 A
        inner join 轄管分局資訊 B on A.分局代碼 = B.轄管分局代碼
    where 
        A.人數 > '1000';
    
--4-2
select distinct B.轄管分局, B.分局電話, count(A.分局代碼) as 轄管設施數量
    from
     設施資訊 A
        inner join 轄管分局資訊 B on A.分局代碼 = B.轄管分局代碼
    where 
...
select C.村里別, A.地址 as 避難設施地址, A.人數 as 容人數量, 轄管分局, B.分局電話 as 分局連絡電話
    from
     設施資訊 A
        inner join 轄管分局資訊 B on A.分局代碼 = B.轄管分局代碼
        inner join 轄管村里資訊 C on A.村里代號 = C.村里別代號
    where
     A.地址
        like '%中%';
